...
for m in row_matches:
    ...
    if ticker in prices:
        ...
        if len(tds) >= 7:
            ...
            new_td2_inner = re.sub(r'>[\d,]+<', f'>{last_price}<', td2_inner)
            # Or if it's just text
            if '>' not in new_td2_inner:
                new_td2_inner = last_price
            
            # Replace Change
            # Update class first
            if '+' in change:
                td3_inner = re.sub(r'change-[a-z]+', 'change-positive', td3_inner)
            elif '&minus;' in change:
                td3_inner = re.sub(r'change-[a-z]+', 'change-negative', td3_inner)
            else:
                td3_inner = re.sub(r'change-[a-z]+', 'change-neutral', td3_inner)
                
            # Replace value
            change_val = change if change != '0.0%' else '0.0%'
            new_td3_inner = re.sub(r'>[^<]+<', f'>{change_val}<', td3_inner)
            
            # Replace Volume
            new_td4_inner = re.sub(r'>[\d,]+<', f'>{volume}<', td4_inner)
            
            # Replace Turnover (span inside div)
            # <td class="text-right"><div ...><span>OLD_VAL</span><span class="invisible-spacer">...
            ...
            for i, td_m in enumerate(tds):
                ...
                if i == 2:
                    new_tr_inner += td_m.group(1) + new_td2_inner + td_m.group(3)
                elif i == 3:
