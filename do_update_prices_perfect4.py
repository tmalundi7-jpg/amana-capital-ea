...
for m in row_matches:
    ...
    if ticker in prices:
        ...
        if len(tds) >= 7:
            td3_open = tds[3].group(1)
            td3_inner = tds[3].group(2)
            td4_open = tds[4].group(1)
            td4_inner = tds[4].group(2)
            td5_open = tds[5].group(1)
            td5_inner = tds[5].group(2)
            td6_open = tds[6].group(1)
            ...
            new_td4_open = re.sub(r'change-(positive|negative|neutral)', c_class, td4_open)
            ...
            for i, td_m in enumerate(tds):
                ...
                if i == 3: new_tr_inner += td3_open + new_td3_inner + td_m.group(3)
                elif i == 4: new_tr_inner += new_td4_open + new_td4_inner + td_m.group(3)
                elif i == 5: new_tr_inner += td5_open + new_td5_inner + td_m.group(3)
                elif i == 6: new_tr_inner += td6_open + new_td6_inner + td_m.group(3)
...
with open('current-prices.html', 'w', encoding='utf-8') as f:
    f.write(new_content)
print("Updated current-prices.html perfectly")
