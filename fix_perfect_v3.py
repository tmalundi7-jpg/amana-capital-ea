...
# Ensure we only replace exactly what is in 4bd9baf
old_gainers = '''<div class="snapshot-mover" id="home-gainers" style="display: flex; flex-direction: column; gap: 0.25rem;">
...
c = c.replace(old_gainers, gainers_repl)
...
old_losers = '''<div class="snapshot-mover" id="home-losers" style="display: flex; flex-direction: column; gap: 0.25rem;">
            <span>VODA <span style="color:var(--loss)">-1.9%</span></span>
            <span>TICL <span style="color:var(--loss)">-2.9%</span></span>
            <span>SWIS <span style="color:var(--loss)">-0.4%</span></span>
          </div>'''
...
old_gainers_mi = '''<!-- GAINERS_START -->
      ...
      <!-- GAINERS_END -->'''
c = c.replace(old_gainers_mi, gainers_repl_mi)
...
old_losers_mi = '''<!-- LOSERS_START -->
      <div style="display:flex; flex-direction:column; gap:0.15rem; width:100%;">
        <div style="display:flex; justify-content:space-between; font-size:0.75rem; font-weight:700; color:var(--cream);"><span>VODA</span> <span style="color:var(--loss)">-1.9%</span></div>
        <div style="display:flex; justify-content:space-between; font-size:0.75rem; font-weight:700; color:var(--cream);"><span>TICL</span> <span style="color:var(--loss)">-2.9%</span></div>
        <div style="display:flex; justify-content:space-between; font-size:0.75rem; font-weight:700; color:var(--cream);"><span>SWIS</span> <span style="color:var(--loss)">-0.4%</span></div>
      </div>
      <!-- LOSERS_END -->'''
...
print("Updated perfectly from clean checkout.")
