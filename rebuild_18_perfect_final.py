...
if i < len(paragraphs):
    ...
    for r_idx, row in enumerate(t0[1:]):
        ...
        if '+' in change_text:
            ...
            change_text = change_text.replace('-', '&minus;').replace('−', '&minus;').replace('', '&minus;')
...
if i < len(paragraphs):
    ...
    # We should add paragraph 9 (Gainers:...) before the table if it exists
    if i < len(paragraphs) and ('Gainers:' in paragraphs[i] or 'Losers:' in paragraphs[i]):
        text = paragraphs[i].replace('-', '&minus;').replace('−', '&minus;').replace('', '&minus;')
        # split by newline if there's any
        parts = text.split('\n')
        for p in parts:
...
if i < len(paragraphs):
    ...
    html_parts.append(f'<li><strong>Upper limit:</strong> {paragraphs[i].replace("Upper limit:", "").replace("", "&times;").replace("x", "&times;")}</li>')
    i += 1
    html_parts.append(f'<li><strong>Lower limit:</strong> {paragraphs[i].replace("Lower limit:", "").replace("", "&times;").replace("x", "&times;")}</li>')
    i += 1
...
if match:
    ...
    with open(r'C:\Users\tmalu\.gemini\antigravity\scratch\Amana-capital-ea\dse-wrap-2026-09-18.html', 'w', encoding='utf-8') as f:
        f.write(full_html)
    print("Perfect rebuild complete with Gainers text fixed!")
else:
