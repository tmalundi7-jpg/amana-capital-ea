...
def process_wrap(day):
    ...
    for p in soup.find_all('p'):
        ...
        # Check if the entire paragraph is just a strong tag
    ...
    <h3 style="color: var(--navy); margin-top: 0; font-size: 1.4rem; margin-bottom: 1.5rem;">{text}</h3>
                ...
                if next_p:
                    next_p.extract()
                    # Do NOT add inline style to the paragraph inside section 7!
                    # next_p['style'] = "color: var(--navy); line-height: 1.8;"
                    new_div.append(next_p)
            else:
                # Do NOT add inline style to the paragraph
                pass
        else:
            # Do NOT add inline style to normal paragraphs
            pass
    ...
    # Update canonical URL in prefix
