...
window.initBondCalculator = function() {
    ...
    function generateStressMatrix(baseYield, targetPeriods, targetRedemption, fv, cr, ppy, dslc) {
        ...
        shocks.forEach((s, i) => {
            ...
            const bpsColor = bps < 0 ? 'var(--loss)' : 'var(--gain)';
            const impactColor = s > 0 ? 'var(--loss)' : 'var(--gain)'; // positive yield shock = negative price impact
            
            const rowBg = i % 2 !== 0 ? 'background: rgba(255,255,255,0.02);' : 'background: transparent;';
            html += `<tr style="${rowBg}">
                <td style="padding: 0.5rem; border-bottom: 1px solid rgba(255,255,255,0.05); color: ${bpsColor};">${sign}${bps} bps</td>
                <td style="padding: 0.5rem; text-align:right; border-bottom: 1px solid rgba(255,255,255,0.05); color: ${impactColor}; font-weight:600;">${sign}${(s * 100).toFixed(1)}%</td>
                <td style="padding: 0.5rem; text-align:right; border-bottom: 1px solid rgba(255,255,255,0.05); color: var(--gain);">${pctPriceStr}</td>
            </tr>`;
        });
        return html;
    }
...
window.initDSEHeatmap = function() {
    ...
    if (!container) return;
                ...
                const data = [{symbol:'NMB',marketCap:2675,change:0.0},{symbol:'TBL',marketCap:3200,change:0.0},{symbol:'CRDB',marketCap:1515,change:2.4},{symbol:'VODA',marketCap:1200,change:-0.8},{symbol:'TPCC',marketCap:900,change:1.8},{symbol:'NICO',marketCap:700,change:-3.5},{symbol:'KCB',marketCap:380,change:0.0},{symbol:'TCC',marketCap:340,change:0.2},{symbol:'TCCL',marketCap:300,change:-2.8},{symbol:'DCB',marketCap:180,change:-4.5},{symbol:'MKCB',marketCap:150,change:1.4},{symbol:'TOL',marketCap:120,change:1.1},{symbol:'SWIS',marketCap:100,change:1.2},{symbol:'AFRIPRISE',marketCap:80,change:0.6},{symbol:'MCB',marketCap:50,change:-1.3},{symbol:'PAL',marketCap:40,change:0.0},{symbol:'MUCOBA',marketCap:20,change:11.1},{symbol:'MBP',marketCap:18,change:2.9},{symbol:'DSE',marketCap:16,change:0.5},{symbol:'TTP',marketCap:12,change:0.0}];

    const totalCap = data.reduce((sum, item) => sum + item.marketCap, 0);
    
    let html = '<div style="display: flex; flex-wrap: wrap; align-content: stretch; width: 100%; height: 100%; gap: 2px;">';
    
    data.forEach(item => {
        let pct = (item.marketCap / totalCap) * 100;
        let color = '#334155'; // Neutral grey-blue
        if (item.change > 1.5) color = '#16a34a';
        else if (item.change > 0) color = 'rgba(22,163,74,0.85)';
        else if (item.change < -1.5) color = '#dc2626';
        else if (item.change < 0) color = 'rgba(220,38,38,0.85)';
        
        let sign = item.change > 0 ? '+' : '';
        // Flex-grow based on marketCap makes them proportionally sized!
        html += `<div style="flex-grow: ${item.marketCap}; flex-basis: ${Math.max(10, pct)}%; background-color: ${color}; 
                      display: flex; flex-direction: column; align-items: center; justify-content: center; 
                      min-height: 85px; padding: 6px; box-sizing: border-box; overflow: hidden; border-radius: 0px; 
                      border: 1px solid rgba(0,0,0,0.2);">
                     <span style="color: #ffffff; font-weight: 700; font-family: 'Plus Jakarta Sans', sans-serif; font-size: 16px; letter-spacing: 0.5px;">${item.symbol}</span>
                     <span style="color: rgba(255,255,255,0.9); font-family: 'Plus Jakarta Sans', sans-serif; font-size: 14px; font-weight: 600;">${sign}${item.change.toFixed(2)}%</span>
                 </div>`;
    });
