$dir_path = "C:\Users\tmalu\.gemini\antigravity\scratch\amana-repo\amana-capital-ea-main"
$files = Get-ChildItem -Path $dir_path -Filter "*.html" -Recurse
...
foreach ($f in $files) {
    ...
    $new_content = $content -replace '(?s)<div class="social-links">.*?</div>', $new_social_links
    if ($content -ne $new_content) {
        Set-Content -Path $f.FullName -Value $new_content -Encoding UTF8
