...
#     <div ...>...</div>
# </div>
# So we need to match from `<div style="display: flex; gap: 1rem; flex-wrap: wrap;">` until the closing `</div>` of the flex container.
# We can do this safely:

c = re.sub(r'<div style="display: flex; gap: 1rem; flex-wrap: wrap;">(.*?)</div>\s*</div>\s*</div>\s*</div>\s*</div>', new_blocks, c, flags=re.DOTALL)
