...
# Corrected version - handles actual structure of block trades section
...
# The current structure is: <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
# followed by individual divs for each block trade, closed by </div>
# Find the section by its heading marker

# Build the block trade cards
block_cards_html = ''
...
# Replace the flex container with block trades
new_flex_block = (
...
# Replace existing flex block trades container
content = re.sub(
    r'<div style="display: flex; gap: 1rem; flex-wrap: wrap;">.*?</div>',
    new_flex_block,
    content,
    flags=re.DOTALL
)
...
checks = {
    ...
    'TBL block trade 307,000': '307,000' in content,
    'VODA block trade 1,550,000': '1,550,000' in content,
    'IEACLC-ETF block trade 215,469': '215,469' in content,
    'VODA +3.5%': '+3.5%' in content,
    'TCCL -5.1%': '-5.1%' in content,
    'MUCOBA -6.8%': '-6.8%' in content,
...
if all_ok:
    ...
    # Debug: show block trades section
    m = re.search(r'Pre-Arranged Board.*?</div>', content, re.DOTALL)
    if m:
        print('\nCurrent block trades section:')
        print(repr(content[m.start():m.start()+600]))
