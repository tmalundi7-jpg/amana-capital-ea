...
if end_idx == -1:
    ...
    # Backup slightly
    temp_idx = content.rfind('</div>', start_idx, end_idx)
    temp_idx = content.rfind('</div>', start_idx, temp_idx)
    end_idx = temp_idx + 6

print('Replacing block trades from', start_idx, 'to', end_idx)
if start_idx != -1 and end_idx != -1:
    content = content[:start_idx] + new_flex_block + content[end_idx:]
